...
flags into `RydbergSystem.set_atom_level`:

```python
from ryd_gate import level_structure

spec = level_structure("rb87_7_mp")
system = (RydbergSystem
          .set_atom_level(spec, **spec.physical_kwargs(), **noise.physical_kwargs())
          .set_atom_geom(Register.chain(2, spacing_um=3.0)))
```

`validate_for(backend=..., level_structure=..., n_atoms=...)` reports
capability errors per backend (decay needs `analog_3`, `rb87_7_mp`, or
`rb87_7_pm`; no TN
