...
def build_pulse(Proto, x, fixed):
    """Concrete TO/AR pulse from the non-theta entries of the optimizer x-vector.

    x layouts: TO [A, w, phi0, d, theta, T] (theta = x[4]);
    AR [w, A1, p1, A2, p2, d, T, theta] (theta = x[7]). ``fixed`` carries the
    manifold's Rabis, intermediate detuning and rise time.
    """
...
def average_gate_infidelity(system, Proto, fixed, x, backend_options=None):
    ...
    bound = system.with_protocol(build_pulse(Proto, x, fixed))
    corrections = {"00": 1.0, "01": np.exp(-1j * theta),
                   "11": np.exp(-2j * theta - 1j * np.pi)}
    a = {k: corrections[k] * simulate(
             bound, labels, backend_options=backend_options).amplitude(labels)
...
# manifold (formerly protocol/preset defaults; TO/AR now take them explicitly,
# P19/P20). mp is σ⁻/σ⁺ (491/185 MHz, Delta=+9.1 GHz, the dark branch); pm is
# σ⁺/σ⁻ (237/303 MHz, Delta=+7.8 GHz); both use the 20 ns rise.
FIXED_MP = dict(
    intermediate_detuning_rad_s=2 * pi * 9.1e9,
    omega_420_max_rad_s=2 * pi * 491e6,
    omega_1013_max_rad_s=2 * pi * 185e6,
    rise_time_s=20e-9,
)
FIXED_PM = dict(
    intermediate_detuning_rad_s=2 * pi * 7.8e9,
    omega_420_max_rad_s=2 * pi * 237e6,
    omega_1013_max_rad_s=2 * pi * 303e6,
    rise_time_s=20e-9,
)

# (name, level-structure tag, protocol_cls, fixed protocol physics, seed)
GATES = [
    ("to_dark", "rb87_7_mp", TOProtocol, FIXED_MP, X_TO_DARK),
    ("ar_our", "rb87_7_mp", ARProtocol, FIXED_MP, X_AR_OUR),
    ("ar_lukin", "rb87_7_pm", ARProtocol, FIXED_PM, X_AR_LUKIN),
]

for name, atom_tag, Proto, fixed, seed in GATES:
    sys = RydbergSystem(level_structure=level_structure(atom_tag),
                        register=Register.chain(2, spacing_um=3.0),
                        protocol=build_pulse(Proto, seed, fixed))
    print(f"\n=== {name} ===", flush=True)

    # Theta-projection warm start + Nelder-Mead polish, written out.
    f = lambda xv: average_gate_infidelity(sys, Proto, fixed, list(xv))
    ...
    inf_default = average_gate_infidelity(sys, Proto, fixed, xf)
    inf_tight = average_gate_infidelity(
        sys, Proto, fixed, xf, backend_options={"rtol": 1e-9, "atol": 1e-13})
