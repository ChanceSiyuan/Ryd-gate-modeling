...
def run_error_source(label, detuning_sign, x, **system_kwargs):
    ...
    has_decay = (
        system_kwargs.get("enable_rydberg_decay", False)
        or system_kwargs.get("enable_intermediate_decay", False)
        or system_kwargs.get("enable_polarization_leakage", False)
    )
    budget = None
    if has_decay:
...
def main():
    ...
    for sign, sign_label, x in [
        ...
        inf_mid, budget_mid = run_error_source(
            f"intermediate decay ({sign_label})",
            sign,
            x,
            enable_rydberg_decay=False,
        ...
        inf_mid_no_scat, budget_mid_no_scat = run_error_source(
            f"intermediate decay no |0⟩ scattering ({sign_label})",
            sign,
            x,
            enable_rydberg_decay=False,
            enable_intermediate_decay=True,
        ...
        inf_pol, budget_pol = run_error_source(
            f"polarization leakage ({sign_label})",
            sign,
            x,
            enable_rydberg_decay=False,
            enable_intermediate_decay=False,
        ...
        inf_all, budget_all = run_error_source(
            f"all deterministic ({sign_label})",
            sign,
            x,
            enable_rydberg_decay=True,
            enable_intermediate_decay=True,
    ...
    print(f"{'=' * w}")
    print(f"{'Error source':<35} {'Infidelity':>12} {'Contrib.':>12} {'XYZ':>12} {'AL':>12} {'LG':>12}")
    print(f"{'-' * w}")
    ...
    for sl in results:
        ...
        print(
            f"{'Rydberg decay (' + sl + ')':<35} {r['rydberg']:>12.6e} {contrib:>+12.6e}"
        ...
        print(
            f"{'Intermediate decay (' + sl + ')':<35} {r['intermediate']:>12.6e} {contrib:>+12.6e}"
            f" {b['XYZ']:>12.6e} {b['AL']:>12.6e} {b['LG']:>12.6e}"
        )
        ...
        print(
            f"{'Polarization leak (' + sl + ')':<35} {r['polarization']:>12.6e} {contrib:>+12.6e}"
        ...
        print(
            f"{'All deterministic (' + sl + ')':<35} {r['all_det']:>12.6e} {contrib:>+12.6e}"
