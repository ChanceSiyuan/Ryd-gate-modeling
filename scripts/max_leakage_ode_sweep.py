...
from sweeplib import (
    ...
    ProvenanceColumns, PointRecord, Runner, Batch, set_worker_context,
    cli, PlotSpec, render_panel_grid,
)
...
def _pilot_report_extra(runner: Runner) -> dict:
    return {"omega_1013_over_2pi_MHz":
            float(runner.manifest["omega_1013_rad_s"]) / TAU / 1e6}
...
def stage_pilot(runner: Runner,
    ...
    return sweeplib.campaign.stage_pilot(
        ...
        report_extra=_pilot_report_extra)


# ── Stage orchestration and CLI commands ─────────────────────────────────────

def _parse_panels(args) -> set[tuple[int, int]] | None:
    return sweeplib.campaign.parse_panels(
        args, len(DELTA_E_GHZ), len(T_GATE_US))


def _campaign_gammas(cfg: ScanConfig, checks: dict) -> dict:
    return _per_panel_gammas(cfg, checks["decay_rates_rad_s"])
...
def _campaign_hooks() -> sweeplib.campaign.CampaignHooks:
    return sweeplib.campaign.CampaignHooks(
        ...
        row_description=lambda cfg: (
            f"rows {list(cfg.delta_e_ghz)} GHz x cols "
            f"{list(cfg.t_gate_us)} us"))
...
def write_summary_reports(store: Store) -> None:
    sweeplib.campaign.write_summary_reports(store, "omega_420_mhz")

def _status_header(manifest: dict) -> str:
    return (f"Omega_1013/2pi = "
            f"{manifest['omega_1013_over_2pi_MHz']:.6f} MHz  ")

def cmd_status(args) -> None:
    sweeplib.campaign.print_status(
        Store(args.output), all_keys=all_keys, pilot_keys=pilot_keys,
        level_sizes=LEVEL_SIZES, header_extra=_status_header)
