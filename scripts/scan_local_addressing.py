...
def cmd_noise(args):
    ...
    for name, cfg in scans.items():
        ...
        for val in cfg["values"]:
            ...
            pin, xtalk, leak = evaluate_addressing(system, initial_state, protocol, x, kwargs, args.n_mc)
            pin_errs.append(pin)
...
def _simulate_grid_point(task):
    ...
    protocol = SweepProtocol(addressing={0: delta_A, 1: delta_B}, ac_stark_shift=ac_stark_peak)
...
def _run_optimize_scan(args):
    ...
    if getattr(args, "no_ac_stark", False):
        ac_stark_peak = 0.0
        print("  AC-Stark feed-forward DISABLED via --no-ac-stark (diagnostic mode)")
    ...
    if args.sweep_delta is not None:
        ...
        print(
            f"  Δ_max axis: {len(delta_halfs)} values from "
    ...
    if args.sweep_tgate is not None:
        ...
        print(f"  T_gate axis: {len(t_gates_us)} values from {t_gates_us[0]:.2f} to {t_gates_us[-1]:.2f} μs")
    else:
    ...
    dtype = [
        ...
        ("delta_A_over_V", float),
        ("delta_half_mhz", float),
        ("t_gate_us", float),
    ]
    ...
    print(
        f"  Scanning {len(wls)}×{len(powers)}×{len(delta_halfs)}×"
    ...
    tasks = [
        (i, float(wl), float(p), float(d), float(t))
        for i, (wl, p, d, t) in enumerate(
            (wl, p, d, t) for wl in wls for p in powers for d in delta_halfs for t in t_gates_us
        )
    ]
    ...
    def _print_point(done, row):
        ...
        dh, tg = row[17], row[18]
        print(
            f"  [{done:>5}/{n_total}] λ={wl:7.2f} nm  P={power_uw:6.1f} μW  "
    ...
    print(
        f"    delta_A = {best['delta_A_mhz']:.2f} MHz "
        f"({best['delta_A_over_Omega']:.1f} Omega_eff, "
        f"{best['delta_A_over_V']:.3f} V_ryd)"
    )
    print(f"    P_gg={best['P_gg']:.4f}, P_gr={best['P_gr']:.4f}, P_rg={best['P_rg']:.4f}, P_rr={best['P_rr']:.6f}")
    print(
        f"    pinning_leak={best['pinning_leak']:.4f}, "
    ...
    meta_kv = {
        ...
        "fix_power_uw": (float(args.fix_power_uw) if args.fix_power_uw is not None else "None"),
    }
    header_lines = [f"{k}={v}" for k, v in meta_kv.items()]
    header_lines.append(",".join(grid.dtype.names))
    np.savetxt(csv_path, grid, fmt="%.6e", header="\n".join(header_lines), delimiter=",")
    ...
    if top_k > 0:
        ...
        for rank, ri in enumerate(ranking):
            ...
            print(f"    #{rank + 1}: P_gr={P_gr_dyn:.4f}, norm={norm:.6f}, norm_loss={1 - norm:.4e}")
    ...
    meta = {
        ...
        "wls": wls,
        "powers": powers,
        "delta_halfs": delta_halfs,
        "t_gates_us": t_gates_us,
        "best": best,
        "best_idx": best_idx,
...
def _plot_smoothed(grid, meta, args):
    ...
    ax.plot(
        ...
        mfc="none",
        label=(
            rf"Raw best: $\lambda={best['wl']:.2f}$ nm, "
            rf"$P={best['power_uw']:.1f}\,\mu$W"
        ),
    )
    ax.plot(
        ...
        mew=2,
        label=(
            rf"Smoothed best: $\lambda={best_smooth_wl:.2f}$ nm, "
    ...
    if noise is not None:
        print(
            f"    Gaussian sigma={sigma_smooth:g} pts  ->  "
...
def _plot_components_sweep(grid, meta, args):
    ...
    for ax, (field, title, cmap) in zip(axs.flat, panels):
        ...
        fig.colorbar(im, ax=ax)
    fig.suptitle(
        f"Sweep components — best at {xc}={best[xc]:.3g}, {yc}={best[yc]:.3g}, total_cost={best['total_cost']:.4e}",
        fontsize=12,
    )
    fig.tight_layout()
    path = os.path.join(args.outdir, _prefixed("addressing_opt_components_sweep.png", args.prefix))
    fig.savefig(path, dpi=150)
    plt.close(fig)
...
def cmd_optimize_plot(args):
    ...
    if args.csv is None:
        args.csv = os.path.join(args.outdir, _prefixed("addressing_opt_grid.csv", args.prefix))
    ...
    _fallbacks = dict(distance_um=4.0, waist_um=2.0, t_gate_us=4.5, delta_start_mhz=-40.0, delta_end_mhz=40.0)
    ...
    for key in ("rabi_420_hz", "rabi_1013_hz", "Delta_hz", "ac_stark_peak_mhz", "ac_stark_off"):
        if key in csv_meta:
    ...
    if raw.shape[1] == 17:
        ...
        raise ValueError(f"unexpected CSV column count {raw.shape[1]}; expected 17 (legacy) or 19 (sweep-aware)")
    grid = np.empty(raw.shape[0], dtype=dtype)
    for i, name in enumerate(grid.dtype.names):
        grid[name] = raw[:, i]
    print(f"  Loaded {len(grid)} points from {args.csv} ({raw.shape[1]} columns)")
...
def main():
    ...
    p_ns.add_argument("--n-mc", type=int, default=50, help="MC shots per point (default: 50)")
    p_ns.add_argument("--n-points", type=int, default=8, help="Scan points per noise source (default: 8)")
    p_ns.add_argument("--combined", action="store_true", help="Also run combined noise sweep")
    ...
    p_opt = sub.add_parser("optimize", help="2D wavelength x power optimization")
    p_opt.add_argument("--wl-min", type=float, default=OPT_WL_MIN_NM, help="Lower edge of wavelength sweep [nm]")
    p_opt.add_argument("--wl-max", type=float, default=OPT_WL_MAX_NM, help="Upper edge of wavelength sweep [nm]")
    p_opt.add_argument("--n-wl", type=int, default=OPT_N_WL, help="Wavelength grid resolution")
    p_opt.add_argument(
        "--power-min-uw",
        type=float,
        default=OPT_POWER_MIN_UW,
        help="Min addressing power [μW] (≳ adiabaticity threshold)",
    )
    p_opt.add_argument(
        "--power-max-uw", type=float, default=OPT_POWER_MAX_UW, help="Max addressing power [μW] (limited by scattering)"
    )
    p_opt.add_argument("--n-power", type=int, default=OPT_N_POWER, help="Power grid resolution")
    p_opt.add_argument(
        "--distance-um", type=float, default=OPT_DISTANCE_UM, help="Inter-atom separation [μm]; keeps V_ryd ≫ δ_A"
    )
    p_opt.add_argument(
        "--waist-um", type=float, default=OPT_WAIST_UM, help="Addressing beam 1/e² waist [μm]; sets crosstalk η"
    )
    p_opt.add_argument(
        "--delta-start-mhz", type=float, default=OPT_DELTA_START_MHZ, help="Initial detuning of LZ ramp [MHz]"
    )
    p_opt.add_argument("--delta-end-mhz", type=float, default=OPT_DELTA_END_MHZ, help="Final detuning of LZ ramp [MHz]")
    p_opt.add_argument("--t-gate-us", type=float, default=OPT_T_GATE_US, help="Gate duration [μs] over which Δ ramps")
    p_opt.add_argument("--top-k", type=int, default=OPT_TOP_K, help="Number of best candidates to print + validate")
    p_opt.add_argument("--outdir", type=str, default=OPT_OUTDIR, help="Output directory for CSV + plots")
    p_opt.add_argument(
        "--n-jobs", type=int, default=OPT_N_JOBS, help="CPU workers for grid scan (0 = all cores, 1 = serial)"
    )
    p_opt.add_argument(
        "--prefix",
        type=str,
        default=OPT_PREFIX,
        help="Filename prefix for all CSV/PNG outputs (e.g. 'exp1' → exp1_addressing_opt_grid.csv, ...)",
    )
    p_opt.add_argument(
        "--smooth-sigma",
        type=float,
        default=OPT_SMOOTH_SIGMA,
        help="Gaussian smoothing width in power-grid points for the smoothed landscape",
    )
    p_opt.add_argument(
        "--no-ac-stark",
        action="store_true",
        default=OPT_NO_AC_STARK,
        help="Disable AC-Stark phase feed-forward in SweepProtocol (diagnostic only — see issue #44)",
    )
    ...
    p_opt.add_argument(
        ...
        metavar=("MIN", "MAX", "N"),
        default=None,
        help="Replace fixed --t-gate-us with a sweep over N values from MIN to MAX μs.",
    )
    p_opt.add_argument(
        "--fix-wl",
        type=float,
        default=None,
        help="Pin wavelength to a single value (nm); collapses the wavelength axis to one point.",
    )
    p_opt.add_argument(
        "--fix-power-uw",
        type=float,
        default=None,
        help="Pin power to a single value (μW); collapses the power axis to one point.",
    )
    ...
    p_plt.add_argument(
        ...
        type=str,
        default=None,
        help="Path to grid CSV from optimize run (default: <outdir>/[<prefix>_]addressing_opt_grid.csv)",
    )
    p_plt.add_argument("--outdir", type=str, default=OPT_OUTDIR, help="Output directory for re-plotted figures")
    p_plt.add_argument(
        "--prefix",
        type=str,
        default=OPT_PREFIX,
        help="Filename prefix for output figures (and for auto-resolving --csv if not given explicitly)",
    )
    p_plt.add_argument(
        "--smooth-sigma",
        type=float,
