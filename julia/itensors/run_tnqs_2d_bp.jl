...
function run_2dtn_bp(payload)
    ...
    final_sigma_z = sigma_z_2d(psi_bpc, lattice, measurement_alg, measurement_bond_dim)
...
function record_observables!(
    ...
    sigma = need_sigma ? sigma_z_2d(psi_bpc, lattice, measurement_alg, measurement_bond_dim) : Float64[]
    if "sigma_z" in observables || "z_i" in observables
        push!(obs_sigma_z, sigma)
    end
    if "czz_centerline" in observables
        push!(obs_czz_centerline, centerline_connected_zz(psi_bpc, lattice, sigma, measurement_alg, measurement_bond_dim))
    end
end
