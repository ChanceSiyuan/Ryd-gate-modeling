...
def simulate_tn(
    ...
    if backend == "gputn":
        ...
        from gputn.backend import GPUTNTDVPBackend
    ...
    if backend == "itensors":
        from .compiler import TNEvolutionIR
        from itensor.backend import ITensorsJuliaBackend
    ...
    if backend in {"ttn", "2dtn", "nqs"}:
        ...
        if backend == "2dtn" and _use_tnqs_2dtn_backend(opts):
            from itensor.tnqs_backend import TNQSJulia2DTNBackend
    ...
    if method == "dmrg":
        from tenpy_mps.backends import (
            TenpyDMRGBackend,
