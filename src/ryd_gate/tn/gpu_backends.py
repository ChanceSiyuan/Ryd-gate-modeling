...
class GPUTNTDVPBackend:
    """GPU tensor-network TDVP backend adapter.

    The public adapter is wired into :func:`ryd_gate.tn.simulate_tn` so callers
    can switch from ``backend="tenpy"`` to ``backend="gputn"`` without changing
    protocol/notebook code. The numerical engine is injected through ``engine``
    for now; the engine should expose an ``evolve(...)`` method with the same
    signature used below. This keeps the dispatch and dependency boundary stable
    while a cuTensorNet implementation is developed.
    """
    ...
    def evolve_compiled(
        ...
        if self.engine is None:
            raise NotImplementedError(
                "The GPUTN backend dispatch is available, but no cuTensorNet TDVP "
                "engine is configured yet. Provide `backend_options={'engine': ...}` "
                "or implement the engine inside GPUTNTDVPBackend. The API boundary is "
                "already stable: use `simulate_tn(..., backend='gputn')`."
            )
