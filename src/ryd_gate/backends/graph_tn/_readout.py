...
class _BaseReader:
    ...
    def _coefficient(self, labels) -> complex:
        ...
        raw = complex(np.asarray(psi.isel(sel).contract(output_inds=())).reshape(()))
        if self._norm is None:
            self._norm = float(np.real(psi.H @ psi)) ** 0.5
    ...
    def _statevector(self) -> np.ndarray:
        if self._dense is None:
            ...
            v = np.asarray(psi.to_dense([psi.site_ind_id.format(s) for s in sites])).reshape(-1)
