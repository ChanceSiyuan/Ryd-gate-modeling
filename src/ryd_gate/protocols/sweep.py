...
class SweepProtocol(Protocol):
    ...
    def __init__(
        ...
        scatter_rates: dict[int, float] | None = None,
    ...
    def unpack_params(self, x: list[float], system) -> dict:
        ...
        consumed by exact/TN compilers to build backend-specific overlay
        terms. The protocol owns parameter semantics only.
        """
    ...
    def _get_scatter_map(self) -> dict[int, float]:
        """Resolve per-atom scatter rates from scalar or dict form."""
