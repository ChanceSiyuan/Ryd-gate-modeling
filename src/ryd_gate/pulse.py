"""Pulse envelope kernel helpers.

Continuous-time functions (in seconds) that build the gate protocols'
flat-top envelopes: ``blackman_window`` / ``blackman_pulse`` /
``blackman_pulse_sqrt``. Consumed by :mod:`ryd_gate.protocols.gate_cz`
(TOProtocol / ARProtocol amplitude envelopes).
"""
