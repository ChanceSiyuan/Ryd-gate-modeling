% Canonical repository-owned derivation; see theory/README.md.
\chapter{Numerical simulations}


\section{The full Hamiltonian}
We first summarize the energy levels that involved in the process:
\begin{itemize}
    \item $\{\ket{0},\ket{1},\ket{e_F},\ket{r},\ket{r'}\}$ for two-photon process.
    \item $\{\ket{0},\ket{1},\ket{r_m}\}$ for single-photon process.
\end{itemize}
...
Decomposed the hyperfine state into
\begin{align}
\ket1
&=\frac{1}{\sqrt2}
\left(
...
\right),
\end{align}
we conclude that $\sigma^-, \pi, \sigma^+$ laser will coupling along the path
\begin{align}
&\sigma^-:
\left\{
\begin{aligned}
...
$V_{\rm od}$, and apply Proposition~\ref{prop:Feshbach_second_order} with
$P=P_3$, $Q=Q_4$, $H_P=PH_0P$, and $H_Q=QH_0Q$.  In the notation of
Eq.~\eqref{eq:general_Feshbach_resolvents},
\begin{align}
\ket{\nu_a}
=QV_{\rm od}\ket a
...
\end{align}
Substitution into Eq.~\eqref{eq:general_Feshbach_second_order} gives
...
\begin{figure}[htbp!]
    \centering
    \includegraphics[width = 0.8\textwidth]{pic/GRAPE.png}
\end{figure}
\begin{align}
...
= \delta \left[V_k \ket{\psi_{k+1}^{(s)}}\right]
